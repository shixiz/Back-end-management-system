...
export default {
  ...
  methods: {
    ...
    doRegister() {
      ...
      axios
        .get(
          `http://localhost:8080/shoptest_war_exploded/user/${this.username}`
        )
        .then((res) => {
          if (!res.data) {
            if (
              Boolean(this.ruleForm.username.match(uPattern)) &&
              Boolean(this.ruleForm.password.match(pPattern))
            ) {
              axios
                .post(
                  "http://localhost:8080/shoptest_war_exploded/user/",
                  this.ruleForm
                )
                .then((res) => {
                  this.open2();
